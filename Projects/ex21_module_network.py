...
#3) 데이터 구별을 위해 cel별로. 띄어쓰기를 도입해 봄. 1차 시도(한줄 단위로.. 엑셀의 행을 처리하고,.. 띄어쓰기로 칸들을)
...
#5) csv의 단점.. 값들의 식별자를 알기 어렵다는 문제를 개선한 xml 표기형식(파일형식)
...
# xml 문자열을 분석해주는 별도의 모듈이 등장함. 표준모듈이며 별도설치는 필요 없고, 단지 import만 하면 된다.
import xml.etree.ElementTree as ET

# 최상위 요소(elements)부터 찾아오기
...
# 각 요소 안에 있는 텍스트(글씨)데이터 출력
    print(name.text, age.text, major.text, address.text)
print()
print('-'*40)
print()

#6) XML의 불편한 점.. 시작/종료 태그문이 글자수를 너무 많이 차지함. 그래서 등장한 json 형식
...
# json모듈의 기능 중.. json 형식의 문자열을 파이썬의 dictionary 타입으로 변환해주는 기능 loads()
...
#[별 외] 표준모듈인 request의 단점.[한글깨짐 문제, 예외처리를 직접해야 함]
try:
    address= 'https://raw.githubusercontent.com/2017mrhi/python_network_files/refs/heads/main/aaa.txt'
    address= 'https://raw.githubusercontent.com/2017mrhi/python_network_files/refs/heads/main/aa.txt' #파일명aa 오류.url 오류
    url= request.urlopen(address)
    data= url.read()
    print(data)
    print()
...
# 위의 문제를 해결하기 위해서... 외부에서 개발된 네트워크 전용 모듈을 사용함 requests 외부모듈...소개
