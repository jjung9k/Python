...
# ex) '신림 고등학교' 학생들의 명단을 관리 서비스... 
# 학생마다 [이름, 학년] 묶어서 저장하기 위해 그룹을 만들기
# 추가로.. 학생객체안에 [학교명]정보를 저장하고 싶어.. 모두 같은 학교학생들이면 굳이 학교명을 학생들마다 변수로 가질 필요가 없다.
# Student 클래스로 만들 객체들이 모두 하나의 변수값을 공유하고 싶을때.. [클래스 변수]를 만들어 사용할 수 있음
class Student:
    school_name = '신림 고등학교' # 클래스 변수(데이타를 저장하고 참조) - 클래스에 단 1개만 존재하는 변수
...
# 클래스 변수는 모든 객체들이 공유하여 사용이 가능하기에... 마치 본인이 가진 변수인 것 처럼 사용하는 것도 가능함
...
#객체명을 사용하면 안 되는 이유..
stu2.school_name= '종로 고등학교' #객체명을 이용하여 클래스명 변경을 시도!!!
...
#결론은,,, 클래스 변수를 제어하려면.. 무조건 클래명으로 접근!!
...
class Hello:
    ...
    #클래스 메소드 >> 클라스 메소드가 되려면.. 파이썬 인터프리터에게 '데코레이터 @'를 이용하여 클래스 메소드라고 표기해야 함
    @classmethod
    def output(self):
        print('output!!!!!!')
...
#객체를 생성하지 않아도.. 클래스명을 통해 메소드 호출이 가능
Hello.output()
# 대표적인 사용예시 -- 현재 시간... now() 함수
import datetime
now= datetime.datetime.now()  # now() 클래스메소드
