...
# 흔하게 볼 수 있는 예외 상황들 4가지 정도 알아보고 '예외처리'를 해 보기
#1. 0 나눗셈
# print()
# n= 2

# #예외가 발생할 가능성이 있는 코드에 대해 예외처리 문법을 사용 try - exception 문법
# try:
#     print( int(10/n) )
# except: #예외가 발생하면 실행되는 영역
#     print(' 0 으로 나눗셈은 불가능 해요.')

# #3. 리스트의 인덱스번호를 잘못 사용하는 경우
# aaa= [10, 20, 30]
...
# # 예외발생 가능성 코드만 try 영역에 예외가 아닐때(성공했을때) 수행할 코드를 다른 영역에 작성하여 가독성 향상 - else
# try:
#     #예외발생 가능성 있는 코드
...
# #4) 예외발행 여부와 상관없이 무조건 수행할 작업이 있다면... 사용하는 finally
...
# print('파일스트림이 닫혀 있나요?: ', file.closed )
# # ---------------------------------------------------------------------------
# print()
#5) 예외가 발생했을 때 그 이유를 알고 싶다면...
try:
    print(10/0)
...
try:
    ...
    print('에러이유: ', e)
print()
# 이런식으로 에러의 종류를 알수 있다면.. 에러별로 대응하는 코드로 가능함.
try:
    print(10/0)
    num1= int( input('입력: '))
    num2= int( input('입력: '))
    ...
    print('0의 나눗셈은 불가능 합니다.: ', e)
except Exception as e:
    print('알수 없는 예외가 발생했어요.', e)

print()
#------------------------------------------------------------
#[추가] 예외 상황이 아닌데... 개발자가 강제로 예외야!! 라고 문제를 발생시키는 문법. raise
...
# exception(예외)가 발행하면 그 줄 다음부터의 코드는 실행되지 않음... 이를 확인해 보기
